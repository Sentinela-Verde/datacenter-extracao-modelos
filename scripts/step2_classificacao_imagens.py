...
def parse_args():
    ...
    parser.add_argument('--skip-smote', action='store_true', help='Não aplicar SMOTE para balancear classes')
...
def main():
    ...
    print("\n" + "="*100)
    print(f"SENTINELA VERDE - STEP 2 (Classificação)")
    print("="*100 + "\n")
    
    print(f"Data Center: {args.name}")
    print(f"Ano de Referência: {args.ano_referencia}")
    print()
    ...
    print("2. Preparando rótulos de referência...\n")
    ...
    # Features e amostragem
    print("3. Extraindo features e amostras...\n")
    ...
    # Aplicar SMOTE (opcional)
    if not args.skip_smote:
        print("\n4. Balanceando classes com SMOTE...\n")
    ...
    detailed_metrics_report(y_test, rf_model.predict(X_test))
    
    # XGBoost (se disponível)
    try:
        xgb_model = train_xgboost(X_train, y_train, X_test, y_test)
        detailed_metrics_report(y_test, xgb_model.predict(X_test))
    except Exception as e:
        print(f"⚠️ Erro ao treinar XGBoost: {e}")
        xgb_model = None
    
    # Neural Network
    nn_model, scaler = train_neural_network(X_train, y_train, X_test, y_test)
    detailed_metrics_report(y_test, np.argmax(nn_model.predict(scaler.transform(X_test), verbose=0), axis=1))

    # Classificação da série temporal
    print("\n6. Classificando série temporal...\n")
    ...
    for year in meta['year_list']:
        ...
        for classe in CLASS_NAMES:
            rows.append({
                ...
                'percentual': pct_rf[classe]['percentual'], 
                'area_km2': pct_rf[classe]['area_km2']
            })
        
        # XGBoost se disponível
        if xgb_model is not None:
            classified_xgb = classify_image(fstack, nmask, xgb_model)
            pct_xgb = compute_class_percentages(classified_xgb)
            for classe in CLASS_NAMES:
                rows.append({
                    'ano': year, 
                    'classe': classe, 
                    'modelo': 'XGBoost',
                    'percentual': pct_xgb[classe]['percentual'], 
                    'area_km2': pct_xgb[classe]['area_km2']
                })
        ...
        for classe in CLASS_NAMES:
            rows.append({
                ...
                'classe': classe, 
                'modelo': 'Rede Neural',
                'percentual': pct_nn[classe]['percentual'], 
        ...
        if not args.skip_overlays:
            ...
            plot_mask_overlay(
                ...
                titulo=f'{args.name} {year} - Random Forest', 
                alpha=0.5,
                salvar_em=os.path.join(args.jpg_dir, f'{args.name}_{year}_rf_overlay.jpg'),
                mostrar=False,
            )
            if xgb_model is not None:
                plot_mask_overlay(
                    fstack, classified_xgb,
                    titulo=f'{args.name} {year} - XGBoost', 
                    alpha=0.5,
                    salvar_em=os.path.join(args.jpg_dir, f'{args.name}_{year}_xgb_overlay.jpg'),
                    mostrar=False,
                )
    ...
    print("\n" + "="*100)
    print("✓ STEP 2 CONCLUÍDO COM SUCESSO!")
    print("="*100 + "\n")
